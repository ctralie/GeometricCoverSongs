...
def doImageSublevelsetFiltration(D, noninf = True):
    ...
    I = ripser(sparseDM, distance_matrix=True, maxdim=0)['dgms'][0]
    if noninf:
        I = I[np.isfinite(I[:, 1]), :]
    return I
...
def testBottleneckNoisyCircle():
    ...
    (matchidxb, bdist, bD) = getBottleneckDist(I1, I2)
    btime = time.time() - tic 
    print("Elapsed Time Bottleneck: %.3g"%btime)
